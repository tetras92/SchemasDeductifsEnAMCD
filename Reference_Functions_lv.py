...
def _doPa(covector_to_check, step=0, BDB=None):
    ...
    if all(covector_to_check >= 0) and not (all(covector_to_check == 0)):
        # BDB['[doPa]'][tuple(covector_to_check)] = (True, (prefix + "[doPa]", None, None, 1))
...
#     for i in SigmaP:
#         model_gurobi.addConstr(
#             quicksum([BVar_Dict[j] for j in range(len(PI_statements_List)) if
#                       PI_statements_List[j][0][i] == 1]) <= 1)  # admis i.e. dans B et dans Dm
#
#     # C3
#     for i in SigmaD:
#         model_gurobi.addConstr(
...
#         model_gurobi.addConstr(
#             quicksum([BVar_Dict[j] for j in range(len(PI_statements_List)) if
#                       PI_statements_List[j][1][i] == 1]) == 0)  # couverture au sens strict : mécanisme de couverture
#
#     # model_gurobi.addConstr(quicksum(BVar_Dict.values()) >= 1)  non inDispensable
#
#     model_gurobi.update()
...
#     model_gurobi.optimize()
#     if model_gurobi.status == GRB.OPTIMAL:
#         ElementsDeXRecuperes = {i for j in range(len(PI_statements_List)) if BVar_Dict[j].x == 1 for i in
#                                 range(len(alt_x)) if PI_statements_List[j][0][i] == 1 and i in SigmaP}
#         ElementsDeYRecuperes = {i for j in range(len(PI_statements_List)) if BVar_Dict[j].x == 1 for i in
#                                 range(len(alt_y)) if PI_statements_List[j][1][i] == 1 and i in SigmaD}
#
#         # assert len(ElementsDeXRecuperes) <= len(SigmaP)
#         is_SigmaP_Subset_ElementsDeXRecuperes = len(SigmaP - ElementsDeXRecuperes) > 0
#
#         assert len(SigmaD) == len(ElementsDeYRecuperes)
#         # is_ElementsDeYRecuperes_Subset_SigmaD = len(ElementsDeYRecuperes - SigmaD) > 0
#
#         L = list()
#         for j in range(len(PI_statements_List)):
#             if BVar_Dict[j].x == 1:
...
#                 L.append(("".join([chr(ord('a') + i) for i in set_uj]), "".join([chr(ord('a') + i) for i in set_vj])))
#
#         if is_SigmaP_Subset_ElementsDeXRecuperes:  # element de Dm avant
#             L = ['D'] + L
#
#         details = (prefix + "[cov]", None, L, len(L))
#         BDB['[cov]'][(tuple(alt_x), tuple(alt_y))] = (True, details)
...
def _cov_incomplet_plus_recent(to_explain_statement, PI_statements_List, W, step=0, BDB=None):
    if BDB is None:
        BDB = dict()
    prefix = "" if step == 0 else "->"
    alt_x, alt_y = to_explain_statement[0], to_explain_statement[1]
    ...
    Y_item_Var_Conditionne = {i_: {j: model_gurobi.addVar(vtype=GRB.BINARY, name=f'yvc_{i_}{j}')
                                   for j in range(len(PI_statements_List)) if PI_statements_List[j][1][i_] == 1}
                              for i_ in SigmaD}

    # C0
    for i_ in SigmaD:
        model_gurobi.addConstr(quicksum(Y_item_Var_Conditionne[i_].values()) == Y_item_Var[i_])

    # Complem_Y_item_Var = {i_: model_gurobi.addVar(vtype=GRB.BINARY, name=f'yv_{i_}') for i_ in
    #                       set(range(len(alt_y))) - SigmaD}
    ...
    for i in set(range(len(alt_y))) - SigmaD:
        model_gurobi.addConstr(
            # quicksum([BVar_Dict[j] for j in range(len(PI_statements_List)) if PI_statements_List[j][0][i] == 1]) <= 1) #
            quicksum([BVar_Dict[j] for j in range(len(PI_statements_List)) if PI_statements_List[j][1][
                i] == 1]) <= 1)  # bug corrigé ce 6/10/23 detecté plus tôt mais non repercuté

    # C5
    for j in range(len(PI_statements_List)):
        altxj, altyj = PI_statements_List[j]
        Setxj = {i_ for i_ in range(len(altxj)) if altxj[i_] == 1}
        for i_ in Setxj & set(X_item_Var.keys()):
            model_gurobi.addConstr(BVar_Dict[j] <= X_item_Var[i_])

        Setyj = {i_ for i_ in range(len(altyj)) if altyj[i_] == 1}
        for i_ in Setyj & set(Y_item_Var.keys()):
            if j in Y_item_Var_Conditionne[i_]:
                model_gurobi.addConstr(BVar_Dict[j] == Y_item_Var_Conditionne[i_][j])

    # C5p

    # C6
    # model_gurobi.addConstr(quicksum([(1 - X_item_Var[i_]) * W[i_] for i_ in X_item_Var]) >= quicksum(
    #     [(1 - Y_item_Var[i_]) * W[i_] for i_ in Y_item_Var]) + quicksum(
    #     [Complem_Y_item_Var[i_] * W[i_] for i_ in Complem_Y_item_Var]))

    model_gurobi.addConstr(quicksum([(1 - X_item_Var[i_]) * W[i_] for i_ in X_item_Var]) >= quicksum(
        [(1 - Y_item_Var[i_]) * W[i_] for i_ in Y_item_Var]))
    ...
    model_gurobi.setObjective(
        quicksum(X_item_Var.values()) + quicksum(Y_item_Var.values()),  # + quicksum(Complem_Y_item_Var.values())
        GRB.MINIMIZE)  # Corriger aujourd'hui 13/09/23
    ...
    if model_gurobi.status == GRB.OPTIMAL:
        ...
        #                         range(len(alt_x)) if PI_statements_List[j][0][i] == 1 and i in SigmaP}
        # ElementsDeYRecuperes = {i for j in range(len(PI_statements_List)) if BVar_Dict[j].x == 1 for i in
        #                         range(len(alt_x)) if PI_statements_List[j][1][i] == 1 and i in SigmaD}       # reecriture ce 06/10
        ...
        for j in range(len(PI_statements_List)):
            if BVar_Dict[j].x == 1:
                set_uj = {i for i in range(len(alt_x)) if PI_statements_List[j][0][i] == 1}
                # set_vj = {i for i in range(len(alt_x)) if
                #           PI_statements_List[j][1][i] == 1 and i not in ElementsDeYRecuperes - SigmaD}
                set_vj = {i for i in range(len(alt_x)) if
                          PI_statements_List[j][1][i] == 1}  # sans fioriture ce 06/10
                L.append(("".join([chr(ord('a') + i) for i in set_uj]), "".join([chr(ord('a') + i) for i in set_vj])))

        remaining_pwc = np.zeros(len(alt_x)), np.zeros(len(alt_x))
        # for i in set(range(len(alt_x))) - ElementsDeXRecuperes: # Corriger aujourd'hui 13/09/23
        for i in SigmaP - ElementsDeXRecuperes:  # Corriger aujourd'hui 13/09/23
            remaining_pwc[0][i] = 1

        # for i in set(range(len(alt_y))) - ElementsDeYRecuperes: # Corriger aujourd'hui 13/09/23
        for i in SigmaD - ElementsDeYRecuperes:  # Corriger aujourd'hui 13/09/23
            remaining_pwc[1][i] = 1

        # for i, cvary in Complem_Y_item_Var.items():
        #     if cvary.x == 1:
        #         remaining_pwc[1][i] = 1
...
# def _cov_incomplet_sauv(to_explain_statement, PI_statements_List, W, step=0, BDB=None):
#     if BDB is None:
#         BDB = dict()
#     prefix = "" if step == 0 else "->"
#     alt_x, alt_y = to_explain_statement[0], to_explain_statement[1]
...
#         Setyj = {i_ for i_ in range(len(altyj)) if altyj[i_] == 1}
#         for i_ in Setyj & set(Y_item_Var.keys()):
#             model_gurobi.addConstr(BVar_Dict[j] <= Y_item_Var[i_])
#
#         # for i_ in Setyj & set(Complem_Y_item_Var.keys()):
#         #     # print("in", i_)
#         #     model_gurobi.addConstr(BVar_Dict[j] <= Complem_Y_item_Var[i_])     non indispensable : ce 6/10/23
#
#     # C6
#     # model_gurobi.addConstr(quicksum([(1 - X_item_Var[i_]) * W[i_] for i_ in X_item_Var]) >= quicksum(
#     #     [(1 - Y_item_Var[i_]) * W[i_] for i_ in Y_item_Var]) + quicksum(
...
# def generate_critical_pairs_and_Tm_restreint_sommant_a_m(filename, m):
#     L = flat_CBTO_formated_for_OfflineSimulator(filename, m)
#     L_as_bins = [integer_to_bin(int(alt_int_val), m)[::-1] for alt_int_val in L]
#     L_as_sets = [{i for i in range(len(thebin)) if thebin[i] == '1'} for thebin in L_as_bins]
#     TmTransformed = [({p - 1 for p in pair[0]}, {p - 1 for p in pair[1]}) for pair in Tn(m)]
...
#     STn = [(min(pair, key=lambda x: L_as_sets.index(x)),
#             max(pair, key=lambda x: L_as_sets.index(x))) for pair in TmTransformed]
#     STnRestreint = [(min(pair, key=lambda x: L_as_sets.index(x)),
#                      max(pair, key=lambda x: L_as_sets.index(x))) for pair in TmTransformed if
#                     len(pair[0]) + len(pair[1]) == m]
#     # STnRestreint = [(min(pair, key=lambda x: L_as_sets.index(x)),
#     #                  max(pair, key=lambda x: L_as_sets.index(x))) for pair in TmTransformed if
#     #                 len(pair[0]) >= 2 and len(pair[1]) >= 2]
#
#     CriticalPair = [(L_as_sets[j], L_as_sets[j + 1]) for j in range(len(L_as_sets) - 1) if
#                     (L_as_sets[j], L_as_sets[j + 1]) in STn]  # contigu et disjoints
#     return CriticalPair, STnRestreint


# def generate_critical_pairs_and_Tm_restreint(filename, m):
#     L = flat_CBTO_formated_for_OfflineSimulator(filename, m)
#     L_as_bins = [integer_to_bin(int(alt_int_val), m)[::-1] for alt_int_val in L]
#     L_as_sets = [{i for i in range(len(thebin)) if thebin[i] == '1'} for thebin in L_as_bins]
#     TmTransformed = [({p - 1 for p in pair[0]}, {p - 1 for p in pair[1]}) for pair in Tn(m)]
